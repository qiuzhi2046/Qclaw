...
describe('openclaw upgrade service', () => {
  ...
  it('marks custom out-of-range installs as manual_block against the pinned target', async () => {
    ...
    expect(result.targetVersion).toBe('2026.4.12')
    ...
    expect(result.manualHint).toContain('2026.4.12')
  })
  ...
  it('activates the managed Windows runtime before verifying a qclaw-bundled auto-correction result', async () => {
    ...
    expect(result.ok).toBe(true)
    expect(result.currentVersion).toBe('2026.4.12')
  })
  ...
  itOnDarwin('uses an isolated admin cache path for privileged npm-global upgrades', async () => {
    ...
    expect(adminInstallArgs).toEqual(expect.arrayContaining(['install', '-g', 'openclaw@2026.4.12']))
