...
describe('rankWindowsActiveRuntimeDiscoveryCandidates', () => {
  ...
  it('prefers private runtime when preferPrivate is true', () => {
    const env = buildTestEnv({
      LOCALAPPDATA: 'C:\\Users\\alice\\AppData\\Local',
    })
    ...
    const ranked = rankWindowsActiveRuntimeDiscoveryCandidates(
      ...
      { env, preferPrivate: true }
    )

    expect(classifyWindowsActiveRuntimeSnapshotFamily(ranked[0].snapshot, { env })).toBe('private')
    expect(classifyWindowsActiveRuntimeSnapshotFamily(ranked[1].snapshot, { env })).toBe('external')
  })

  it('preserves external-first when preferPrivate is false', () => {
    const env = buildTestEnv({
      LOCALAPPDATA: 'C:\\Users\\alice\\AppData\\Local',
    })
